var mic = (function() {
  ...
  function detect() {
    ...
    var dbg = 'ctx:' + audioCtx.state + ' | RMS: ' + rms.toFixed(4);
    if (result.freq > 0) {
    ...
    if (result.freq > 0 && result.clarity > CLARITY_THRESHOLD) {
      var note = Math.round(12 * Math.log2(result.freq / 440) + 69);
      var octave = Math.floor(note / 12) - 1;
      var name = NOTE_NAMES[note % 12] + octave;
      dbg += ' | NOTE: ' + name;
      $('#mic-status').text(name + '  ' + result.freq.toFixed(1) + ' Hz  clarity: ' + result.clarity.toFixed(2));
      if (note !== currentNote) {
        if (currentNote >= 0) tonnetz.noteOff(MIC_CHANNEL, currentNote);
